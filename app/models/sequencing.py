...
class DriverPreference(StrEnum):
    OUTWARD_IN = "outward_in"
    INWARD_OUT = "inward_out"


class StartPoint(BaseModel):
    label: str = Field(..., description="Identifier for the driver's current location")
    latitude: float
    longitude: float
...
class SequencingRequest(BaseModel):
    ...
    driver_preference: DriverPreference = Field(
        ...
        description="outward-in: first stop is farthest from the start, last stop is nearest (route "
        "works back inward toward the base); inward-out: first stop is nearest, last stop is "
        "farthest (route works outward away from the base)",
    )


class SequencingLeg(BaseModel):
    from_label: str = Field(..., description="Label of the origin of this leg")
    to_label: str = Field(..., description="Label of the destination of this leg")
    distance_meters: float
    duration_seconds: float | None = Field(
        default=None,
        description="Driving duration for this leg, if durations were provided",
    )
...
class SequencingResponse(BaseModel):
    ...
    stops_order: list[SequencingStop] = Field(
        ...
        "last stop is the fixed last stop)",
    )
    labels_order: list[str] = Field(..., description="Labels of stops in visit order")
    legs: list[SequencingLeg] = Field(
        ...,
        description="Driving legs from the start point through the stops and back to the start point",
    )
